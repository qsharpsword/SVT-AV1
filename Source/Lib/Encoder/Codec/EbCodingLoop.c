...
static void av1_encode_loop(PictureControlSet *pcs_ptr, EncDecContext *context_ptr,
    ...
    if (component_mask == PICTURE_BUFFER_DESC_FULL_MASK ||
        ...
        uint8_t tx_search_skip_flag = context_ptr->md_context->tx_search_level == TX_SEARCH_ENC_DEC
...

    {
        ...
        if (component_mask == PICTURE_BUFFER_DESC_FULL_MASK ||
            ...
            residual_kernel16bit(
                ...
                context_ptr->blk_geom->tx_width[blk_ptr->tx_depth][context_ptr->txb_itr],
                context_ptr->blk_geom->tx_height[blk_ptr->tx_depth][context_ptr->txb_itr]);
            uint8_t tx_search_skip_flag = context_ptr->md_context->tx_search_level ==
                    TX_SEARCH_ENC_DEC
                ? get_skip_tx_search_flag(context_ptr->blk_geom->sq_size, MAX_MODE_COST, 0, 1)
                : 1;
...
EB_EXTERN void av1_encode_pass(SequenceControlSet *scs_ptr, PictureControlSet *pcs_ptr,
    ...
    input_picture = context_ptr->input_samples = (EbPictureBufferDesc *)
    ...
    if (is_16bit && scs_ptr->static_config.encoder_bit_depth > EB_8BIT) {
        ...
        if ((scs_ptr->static_config.ten_bit_format == 1) ||
            ...
            const uint32_t input_luma_offset = ((sb_origin_y + input_picture->origin_y) *
                                                input_picture->stride_y) +
                (sb_origin_x + input_picture->origin_x);
            const uint32_t input_cb_offset = (((sb_origin_y + input_picture->origin_y) >> 1) *
                                              input_picture->stride_cb) +
                ((sb_origin_x + input_picture->origin_x) >> 1);
            const uint32_t input_cr_offset = (((sb_origin_y + input_picture->origin_y) >> 1) *
            ...
            const uint32_t input_luma_offset = ((sb_origin_y + input_picture->origin_y) *
                                                input_picture->stride_y) +
                (sb_origin_x + input_picture->origin_x);
            const uint32_t input_bit_inc_luma_offset = ((sb_origin_y + input_picture->origin_y) *
                                                        input_picture->stride_bit_inc_y) +
                (sb_origin_x + input_picture->origin_x);
            const uint32_t input_cb_offset = (((sb_origin_y + input_picture->origin_y) >> 1) *
                                              input_picture->stride_cb) +
                ((sb_origin_x + input_picture->origin_x) >> 1);
            const uint32_t input_bit_inc_cb_offset = (((sb_origin_y + input_picture->origin_y) >>
                                                       1) *
            ...
            const uint32_t input_cr_offset = (((sb_origin_y + input_picture->origin_y) >> 1) *
                                              input_picture->stride_cr) +
                ((sb_origin_x + input_picture->origin_x) >> 1);
            const uint32_t input_bit_inc_cr_offset = (((sb_origin_y + input_picture->origin_y) >>
                                                       1) *
    ...
    while (blk_it < scs_ptr->max_block_cnt) {
        ...
        if (part != PARTITION_SPLIT &&
            ...
            for (int32_t d1_itr = (int32_t)blk_it + offset_d1;
                ...
                blk_ptr = context_ptr->blk_ptr = &context_ptr->md_context->md_blk_arr_nsq[d1_itr];
                ...
                blk_ptr->delta_qp = 0;
#endif
                context_ptr->md_skip_blk = context_ptr->md_context->blk_skip_decision
                ...
                context_ptr->evaluate_cfl_ep = (disable_cfl_flag == EB_FALSE &&
                                                context_ptr->md_context->chroma_level ==
                                                    CHROMA_MODE_2);
                ...
                if (blk_ptr->prediction_mode_flag == INTRA_MODE) {
                    ...
                    if (pcs_ptr->slice_type != I_SLICE)
                        context_ptr->intra_coded_area_sb[sb_addr] += blk_geom->bwidth *
                            blk_geom->bheight;
                    ...
                    // Transform partitioning free patch (except the 128x128 case)
...

                        {
                            ...
                            if (is_16bit)
                                ref_pic_list0 = ((EbReferenceObject *)pcs_ptr->parent_pcs_ptr
                                                     ->reference_picture_wrapper_ptr->object_ptr)
                                                    ->reference_picture16bit;
                        ...
                        // Calculate Root CBF
                        if (context_ptr->blk_geom->has_uv)
                            blk_ptr->block_has_coeff = (y_has_coeff | u_has_coeff | v_has_coeff)
                                ? EB_TRUE
                                : EB_FALSE;
                        else
                ...
                // Inter
...
#if ENABLE_PR_1133
                    int8_t ref_idx_l0 = context_ptr->md_context
                                            ->md_local_blk_unit[context_ptr->blk_geom->blkidx_mds]
                                            .ref_frame_index_l0;
                    int8_t ref_idx_l1 = context_ptr->md_context
                                            ->md_local_blk_unit[context_ptr->blk_geom->blkidx_mds]
                                            .ref_frame_index_l1;
#else
...
#endif
                        is_blk_skip = md_context_ptr->md_ep_pipe_sb[blk_ptr->mds_idx].skip_cost <=
...
#if ENABLE_PR_1133
                    context_ptr->md_context->md_local_blk_unit[context_ptr->blk_geom->blkidx_mds]
                        .y_has_coeff[0] = EB_FALSE;
                    context_ptr->md_context->md_local_blk_unit[context_ptr->blk_geom->blkidx_mds]
                        .u_has_coeff[0] = EB_FALSE;
                    context_ptr->md_context->md_local_blk_unit[context_ptr->blk_geom->blkidx_mds]
                        .v_has_coeff[0] = EB_FALSE;
#else
                    ...
                    if (context_ptr->md_context
                        ...
                        for (uint16_t tu_it = 0; tu_it < tot_tu; tu_it++) {
                            ...
                            uint8_t uv_pass      = blk_ptr->tx_depth && tu_it ? 0
                                                                         : 1; //NM: 128x128 exeption
                    ...
                    if (context_ptr->md_context
                        ...
                        context_ptr->md_context
                            ->md_local_blk_unit[context_ptr->blk_geom->blkidx_mds]
                            .merge_flag = (is_blk_skip) ? EB_FALSE : EB_TRUE;
                    }
                    ...
                    if (scs_ptr->use_output_stat_file) {
                        ...
                        if (context_ptr->mv_unit.pred_direction == UNI_PRED_LIST_1 ||
                            ...
                            ref_obj_1->stat_struct.referenced_area[sb_index] += width * height *
                                weight;
                            if (origin_x + blk_geom->bwidth > sb_origin_x2 + context_ptr->sb_sz) {
                                sb_origin_x2 = (origin_x / context_ptr->sb_sz + 1) *
                                    context_ptr->sb_sz;
                                sb_origin_y2 = origin_y / context_ptr->sb_sz * context_ptr->sb_sz;
                                sb_index = sb_origin_x2 / context_ptr->sb_sz +
                                    pic_width_in_sb * (sb_origin_y2 / context_ptr->sb_sz);
                                width = origin_x + blk_geom->bwidth - MAX(sb_origin_x2, origin_x);
                                height = MIN(sb_origin_y2 + context_ptr->sb_sz,
                                             origin_y + blk_geom->bheight) -
                                    origin_y;
                                ref_obj_1->stat_struct.referenced_area[sb_index] += width * height *
                            ...
                            if (origin_y + blk_geom->bheight > sb_origin_y2 + context_ptr->sb_sz) {
                                sb_origin_x2 = (origin_x / context_ptr->sb_sz) * context_ptr->sb_sz;
                                ...
                                width = MIN(sb_origin_x2 + context_ptr->sb_sz,
                                            origin_x + blk_geom->bwidth) -
                                    origin_x;
                                height = origin_y + blk_geom->bheight - MAX(sb_origin_y2, origin_y);
                                ref_obj_1->stat_struct.referenced_area[sb_index] += width * height *
                                    weight;
                            }
                            if (origin_x + blk_geom->bwidth > sb_origin_x2 + context_ptr->sb_sz &&
                                origin_y + blk_geom->bheight > sb_origin_y2 + context_ptr->sb_sz) {
                                sb_origin_x2 = (origin_x / context_ptr->sb_sz + 1) *
                                    context_ptr->sb_sz;
                                sb_origin_y2 = (origin_y / context_ptr->sb_sz + 1) *
                                    context_ptr->sb_sz;
            ...
            if (pcs_ptr->parent_pcs_ptr->frm_hdr.allow_intrabc && is_16bit &&
                (context_ptr->bit_depth == EB_8BIT)) {
                ...
                dst = recon_buffer_8bit->buffer_y + pred_buf_x_offest +
            ...
            update_av1_mi_map(
                blk_ptr, context_ptr->blk_origin_x, context_ptr->blk_origin_y, blk_geom, pcs_ptr);
            if (pcs_ptr->update_cdf) {
...
if (dlf_enable_flag && pcs_ptr->parent_pcs_ptr->loop_filter_mode == 1 && total_tile_cnt == 1) {
    ...
    if (pcs_ptr->parent_pcs_ptr->frm_hdr.loop_filter_params.filter_level[0] ||
        ...
        uint8_t last_col = ((sb_origin_x) + sb_width == pcs_ptr->parent_pcs_ptr->aligned_width) ? 1
                                                                                                : 0;
        loop_filter_sb(
            ...
            for (int32_t d1_itr = blk_it + offset_d1; d1_itr < blk_it + offset_d1 + num_d1_block;
                ...
                do {
                    ...
                    if (context_ptr->blk_geom->has_uv) {
                        ...
                        src_ptr = &(((int32_t *)context_ptr->blk_ptr->coeff_tmp
                ...
                //copy recon
                {
                    ...
                    uint8_t *dst_ptr = ref_pic->buffer_y + ref_pic->origin_x +
                        context_ptr->blk_origin_x +
                    ...
                    if (context_ptr->blk_geom->has_uv) {
                        ...
                        dst_ptr = ref_pic->buffer_cb + ref_pic->origin_x / 2 +
                        ...
                        dst_ptr = ref_pic->buffer_cr + ref_pic->origin_x / 2 +
                            ((context_ptr->blk_origin_x >> 3) << 3) / 2 +
                            (ref_pic->origin_y / 2 + ((context_ptr->blk_origin_y >> 3) << 3) / 2) *
                                ref_pic->stride_cr;
