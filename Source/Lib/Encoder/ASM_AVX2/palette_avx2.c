...
void av1_k_means_dim1_avx2(const int *data, int *centroids, uint8_t *indices, int n, int k,
    ...
    for (int i = 0; i < max_itr; ++i) {
        ...
        if (this_dist > pre_dist) {
            eb_memcpy(centroids, pre_centroids, sizeof(pre_centroids[0]) * k);
...
void av1_k_means_dim2_avx2(const int *data, int *centroids, uint8_t *indices, int n, int k,
    ...
    for (int i = 0; i < max_itr; ++i) {
        const int64_t pre_dist = this_dist;
        eb_memcpy(pre_centroids, centroids, sizeof(pre_centroids[0]) * k * 2);
        eb_memcpy(pre_indices, indices, sizeof(pre_indices[0]) * n);
        ...
        if (this_dist > pre_dist) {
            eb_memcpy(centroids, pre_centroids, sizeof(pre_centroids[0]) * k * 2);
            eb_memcpy(indices, pre_indices, sizeof(pre_indices[0]) * n);
            break;
        }
